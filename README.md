...
You should see progress being made in the analysis (it takes about 30 s on my 2014-vintage MacBook Pro). A table and a series of results will be printed to the R Console pane, and graphs appear in the Plots pane. You should see two warning messages, which can be ignored (they are just telling you that initial parameter value estimates were not used and that the number of MCMC chains was not specified, and so JAGS used defaults for both).
