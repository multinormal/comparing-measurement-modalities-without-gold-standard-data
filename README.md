...
Finally, we compare the modalities by computing posterior probabilities that a given parameter for one modality is better than that for another modality. Typically when comparing things based on data, one wants to know not only which thing is best, but also how confident we can be about that. Often, quantities such as P-values or confidence intervals are used. With the Bayesian approach, however, we can often estimate probabilities that are more interpretable. The ideal modality will have slope, intercept, and standard deviation values that do not differ from the ideal values of those parameters. For a general parameter q, the posterior probability of q being better in modality 1 than 2 can be written as P(|q[1] - w| < |q[2] - w|), where q[1] and q[2] are the true values of parameter q for modality 1 and 2 respectively, and w is the ideal value of the parameter. Note that the true values q[1] and q[2] are unknown and we are estimating them from data. This is why the comparison between them must be stated in terms of probabilities. For the slope parameter, w = 1, and for the intercept and standard deviation parameters, w = 0. Given the MCMC samples from the posterior distribution of those parameters, we can estimate such probabilities for all relevant combinations of parameter and modality. These probabilities are estimated and displayed. We see, for example, that P(|a[1] - 1| < |a[2] - 1|) =  0.0045, which is consistent with the true values of a[1] and a[2] being 0.6 and 0.7, respectively (i.e., 0.6 being further from the ideal slope of 1 than 0.7). We also see that P(|b[2]| < |b[3]|) =  0.9925, which is consistent with the true values of b[2] and b[3] being 0.0 and 0.1, respectively. (Note that the subtraction of the ideal value is only displayed for the slope parameter, since it is zero for the other parameters.) We also see that P(|b[1]| < |b[3]|) =  0.47115, which is consistent with the true values of -0.1 and 0.1 having equal magnitudes. Therefore we can make statements along the lines of "The posterior probability that modality 2 has better accuracy than modality 3 is 0.99" and "The posterior probability that modality 1 has better accuracy than modality 3 is approximately 50%."

## Limitations of the method

The method has three limitations, which I do not believe prevent its sensible use:

1. The relationships between the observed and true values are assumed to be linear. While this assumption is a reasonable first approximation, the true form of the relationships may be nonlinear. Such nonlinearity could be modeled, but the parameters of such a model might make it rather difficult to numerically summarize quantities such as accuracy and precision. In such a case, it may be better to plot the inferred nonlinear relationship between the true and estimated quantities for each modality. One approach may be to compare a model that assumes the relationships are linear with another that can assumes nonlinear relationships.
2. The method requires that you assume a distribution for the true (gold standard) value. The Hoppin et al. paper uses simulations to investigate the case where the assumed distribution of the true value does not match the actual distribution of the true value, and demonstrates some robustness to mismatch. In the simulation used here, the parameters of the assumed distribution of the true value are fixed. It would be possible to place priors on them to represent uncertainty on the distribution if its form could be assumed but its parameterization was uncertain. This uncertainty would propagate to the uncertainty on the posterior distribution of the linear model parameters.
